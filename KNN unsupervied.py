...
def superPredict(testData,trainData,trainGender):
    predicted = list()
    for i in range(len(testData)):
        genderM=0
        genderW=0
        neighbours = get_neighbours(trainData,testData[i],3)
        ...
        if(genderM >genderW):
            x=testData[i],'M',genderM,i
            predicted.append(x)
        else:
            x=testData[i],'W',genderW,i
            predicted.append(x)
    return predicted
...
predictlist = superPredict(testData,trainData,trainGender)
trainGenderCopy = trainGender.copy()
trainDataCopy = trainData.copy()
testDataCopy = testData.copy()
trainGenderIndex = list()
...
batchsize = 1
while len(testDataCopy) > batchsize:
    for i in range(batchsize):
        trainGenderCopy.append(predictlist[i][1])
        trainGenderIndex.append(predictlist[i][3])
        trainDataCopy.append(predictlist[i][0])
        del testDataCopy[i]

    predictlist = superPredict(testDataCopy,trainDataCopy,trainGenderCopy)
...
accuracy(testData,testGender,trainData,trainGender)
...
count=0
for i in range(20,len(trainGenderCopy)-1):
    l=testData.index(trainDataCopy[i])
    if(testGender[l]==trainGenderCopy[i]):
        count= count+1
accuracyPred1 = count/len(testData)
print(accuracyPred1)
