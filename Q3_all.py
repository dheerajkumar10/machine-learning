...
X = np.array([
[1.8364155133121, 86.761325377947, 29],
[1.6415085524931, 64.54519197132, 33],
[1.8272381093161, 85.690113965564, 32],
[1.7205160446391, 62.134351399595, 35],
[1.7043475171367, 67.477953273651, 35],
[1.6964881390008, 77.574609746797, 31],
[1.7608353620688, 76.41302071356, 33],
[1.5586588886553, 62.585061233215, 37],
[1.6879314142859, 74.121672329838, 31],
[1.720702471903, 74.705176894624, 39],
[1.7567657037236, 60.106632087594, 38],
[1.8057488144843, 76.187180335799, 28],
[1.7819405798888, 64.395831701986, 32],
[1.643097537795, 55.705208643129, 34],
])
Y = np.array([1, 0, 1, 1, 0, 0, 1, 0, 1, 1, 0, 0, 0, 0])


# In[157]:


X_test = np.array([[ 1.9067150635717, 69.535756117475, 31],
...
def predict(X, b0, b1, b2, b3):
    prediction = np.array([1 / (1 + np.exp(-1*b0 + -1*b1*x[0] + -1*b2*x[1] + -1*b3*x[2])) for x in X])
    return prediction
...
def logistic_regression(X, Y):
    ...
    for epoch in range(epochs):
        y_pred = predict(X, b0, b1, b2, b3)
        b0_gd = -2 * sum((Y - y_pred) * y_pred * (1 - y_pred)) 
        b1_gd = -2 * sum(X[:, 0].reshape(1, 14) * (Y - y_pred) * y_pred * (1 - y_pred)) 
        b2_gd = -2 * sum(X[:, 1].reshape(1,14) * (Y - y_pred) * y_pred * (1 - y_pred))
        b3_gd = -2 * sum(X[:, 2].reshape(1,14) * (Y - y_pred) * y_pred * (1 - y_pred))
        b0 = b0 - L * b0_gd
        b1 = b1 - L * b1_gd
        b2 = b2 - L * b2_gd
        b3 = b3 - L * b3_gd
...
b0, b1, b2, b3 = logistic_regression(X, Y)


# In[160]:


X_test_1= X_test - X_test.mean()
y_pred = predict(X_test_1, b0, b1, b2, b3)
...
scores = cross_val_score(model, X, Y, scoring='neg_mean_absolute_error',
                         cv=cv, n_jobs=-1)
print("linear regression mean" + mean(absolute(scores)))
...
X1 = np.array([
[1.8364155133121, 86.761325377947],
[1.6415085524931, 64.54519197132],
[1.8272381093161, 85.690113965564],
 [1.7205160446391, 62.134351399595],
 [1.7043475171367, 67.477953273651],
 [1.6964881390008, 77.574609746797],
 [1.7608353620688, 76.41302071356],
 [1.5586588886553, 62.585061233215],
 [1.6879314142859, 74.121672329838],
 [1.720702471903, 74.705176894624],
 [1.7567657037236, 60.106632087594],
 [1.8057488144843, 76.187180335799],
[1.7819405798888, 64.395831701986],
 [1.643097537795, 55.705208643129],
 ])
Y1 = np.array([1, 0, 1, 1, 0, 0, 1, 0, 1, 1, 0, 0, 0, 0])


# In[148]:


def predict1(X, b0, b1, b2):
    return np.array([1 / (1 + np.exp(-1*b0 + -1*b1*x[0] + -1*b2*x[1] )) for x in X])

def logistic_regression1(X, Y):
    X = X1 - X1.mean()
    b0 = 0
    b1 = 0
    b2 = 0
...
tb0, tb1, tb2 = logistic_regression1(X1, Y1)
...
X_test_1 = X_test - X_test.mean()
y_pred = predict1(X_test_1, b0, b1, b2)
for i in range(len(y_pred[0])):
    if(y_pred[0][i] >= 0.5):
        y_pred[0][i] = 1
    else:
...
scores = cross_val_score(model, X1, Y1, scoring='neg_mean_absolute_error',
                         cv=cv, n_jobs=-1)
print("linear regression mean with leaveone out" + mean(absolute(scores)))
